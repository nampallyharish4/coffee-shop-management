...
import { 
  Grid, Card, CardContent, Typography, CardActionArea, Box, 
  Paper, Select, MenuItem, FormControl, InputLabel, Button,
  Table, TableBody, TableCell, TableHead, TableRow, 
  Dialog, DialogTitle, DialogContent, DialogActions, TextField, Alert,
  Divider, useTheme, Stack
} from '@mui/material';
import {
...
const Dashboard = () => {
  ...
  const [salesSummary, setSalesSummary] = useState({ totalRevenue: 0, orderCount: 0, averageOrderValue: 0 });
  ...
  const handleAction = async () => {
    ...
    if (confirmText !== requiredText) {
      setAlert({ open: true, message: `Please type "${requiredText}" to confirm`, severity: 'error' });
      return;
    }
    ...
    try {
      ...
      setAlert({ open: true, message: error.response?.data?.message || 'Failed to perform action', severity: 'error' });
    }
  };
  ...
  const cards = [
    { title: 'POS', icon: <ShoppingCart />, path: '/pos', roles: ['ROLE_CASHIER', 'ROLE_ADMIN'], color: '#6F4E37', desc: 'New Orders' },
    { title: 'Orders', icon: <Receipt />, path: '/orders', roles: ['ROLE_CASHIER', 'ROLE_BARISTA', 'ROLE_ADMIN'], color: '#D2691E', desc: 'Track History' },
    ...
    <Layout title="Dashboard">
              ...
              <Grid item xs={12} sm={6} md={4} lg={2} key={card.title}>
                ...
                >
                  <CardActionArea onClick={() => navigate(card.path)} sx={{ p: 2 }}>
                    <Box sx={{ color: card.color, mb: 1.5, display: 'flex', alignItems: 'center', justifyContent: 'center', width: 45, height: 45, borderRadius: '12px', backgroundColor: `${card.color}15` }}>
                      {card.icon}
                    </Box>
                    <Typography variant="subtitle1" fontWeight="800">{card.title}</Typography>
      ...
      {hasRole('ROLE_ADMIN') && (
        <Box sx={{ mt: 4, animation: 'fadeIn 0.8s ease' }}>
          <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems="center" spacing={2} sx={{ mb: 3 }}>
            <Box>
              <Typography variant="h5" fontWeight="800">Analytics & Reports</Typography>
          ...
          {alert.open && (
            <Alert severity={alert.severity} onClose={() => setAlert({ ...alert, open: false })} sx={{ mb: 3, borderRadius: '12px' }}>
              {alert.message}
            </Alert>
          )}
                ...
                <Box sx={{ height: 350, width: '100%' }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={topItems} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                      ...
                      />
                      <Bar dataKey="totalQuantity" fill={theme.palette.primary.main} radius={[8, 8, 0, 0]} barSize={40} />
                    </BarChart>
                  </ResponsiveContainer>
                </Box>
              </Paper>
      ...
      >
        <DialogTitle sx={{ fontWeight: 800 }}>
            {dialogType === 'RESET_REVENUE' ? 'Reset Revenue Session' : 'Wipe System History'}
        </DialogTitle>
        <DialogContent>
          ...
          <TextField
            fullWidth
            variant="outlined"
            placeholder={dialogType === 'RESET_REVENUE' ? 'RESET REVENUE' : 'DELETE EVERYTHING'}
            value={confirmText}
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          ...
          <Button onClick={handleAction} variant="contained" color="error" sx={{ borderRadius: '10px', boxShadow: 'none', px: 3 }}>
            Confirm Action
          </Button>
        </DialogActions>
      </Dialog>
